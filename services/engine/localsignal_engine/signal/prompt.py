...
def _prompt(signal: dict, evidence: list, place_context: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    ...
    return {
        "instructions": [
            ...
            "why_this_matters must add information that is not already in short_summary: connect the change to a concrete local context such as timing, occasion, neighborhood pattern, or what would confirm or weaken the signal next week. Never restate the summary in different words, and never end with a generic clause like 'suggesting growing community interest'.",
            "When computed_metrics include current_mention_count and baseline_mention_count, cite the actual numbers once in short_summary or ai_summary. Do not use vague quantifiers like surged, spiked, or notable rise without the underlying numbers.",
            "current_mention_count covers current_window_days and baseline_mention_count covers baseline_window_days — different window lengths. Never present the two raw counts as a from-to pair. Convert to the same per-week basis before describing direction (e.g. '5 mentions this week vs roughly 3 per week over the prior month'), so growth never reads as decline.",
